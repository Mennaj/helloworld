...
/*   Updated: 2023/03/13 17:55:15 by mennaji          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_line(t_data *data, t_params *params){
	float dx;
	float dy;
	float steps;

	dx = abs(params->pixel_x2 - params->pixel_x1);
	dy = abs(params->pixel_y2 - params->pixel_y1);
	if(dx>=dy)
		steps = dx;
	else
		steps = dy;
	dx /= steps;
	dy /= steps;
	data->x = params->pixel_x1;
	data->y = params->pixel_y1;
	pixel_put(data, data->x, data->y, RED);
	int i = 0;
	while(i < steps)
	{
...
int fract_select(int argc, char **argv, t_data *d){
...
int key_hook(int keycode, t_data *data)
{
	...
	{
		data->params.b += 1; // down arrow key
		whichfractal(data);
	}
	else if(keycode == 126)
	{
		data->params.b -= 1;// up arrow key
		wichfractol(data);
	}
	else if(keycode == 123)
	{
